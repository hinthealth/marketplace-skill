...
2. **What type of surface?** How should the app appear in Hint:
   ...
   - **Clinical Interaction** (`clinical_interaction`) — appears within clinical workflows, in the context of a specific patient/interaction. Best for clinical tools, lab viewers, and patient-specific features. Receives patient context via `HintSDK.currentPatient` and `HintSDK.interaction`. Set `interaction_type` to choose where it opens: `general` (the default) starts a standalone partner interaction from the command palette and the Interactions widget; `lab` appears as a tab inside the Lab Order interaction instead. An app may register one of each, so a lab tool can also offer a general entry point.
...
> **Clinical surface sizing.** Clinical surfaces render inside a note window that sizes the iframe **only** from the app's `resized` messages. Include `hint-sdk.js` on every embedded page (it auto-reports height via ResizeObserver) — an SDK-less page is clipped to a ~150px strip. Also applies to `core_page` surfaces with auto-adjust height disabled.

> **Testing on a real phone.** `localhost` URLs are useless from a phone, and plain LAN IPs fail twice (mixed content + insecure context disables `navigator.mediaDevices`). Working recipe: `cloudflared tunnel --url http://localhost:<port>`, then point the app's real `handshake_url` and surface `source_url` at the https tunnel URL — no localhost mode needed, works from any network, and secure-context APIs (camera/mic/geo) all function. Caveats: the tunnel URL is ephemeral (re-PATCH after every restart) and publicly reachable while up.
...
> **Browser capabilities (`browser_allow_list`).** If any surface needs a gated browser capability — the device camera via `navigator.mediaDevices.getUserMedia` (live photo capture, barcode scanning, video), microphone, or geolocation — set `browser_allow_list` on the **app** (not the surface): `PATCH $HINT_API_URL/api/partner/products/$PRODUCT_ID/app` with `{"app": {"browser_allow_list": ["camera", "microphone"]}}`. Supported values: `camera`, `microphone`, `geolocation`; defaults to `[]`. Practices/partners can also toggle these in the portal under App Settings → Edit App → "Browser Capabilities". Hint renders every embed iframe for the app with a matching Permissions Policy `allow` attribute, which browsers require before the capability can even prompt inside a cross-origin iframe. The end user still sees the browser's normal permission prompt — the list delegates the capability, it does not grant it. The `allow` attribute is rendered at embed time, so changes only apply to newly opened surfaces — reload any already-open surface after changing the list. Without it, getUserMedia is rejected immediately (`NotAllowedError`) with no prompt. The handshake payload echoes the current `browser_allow_list` — preflight against it before calling getUserMedia so you can tell "not delegated by Hint" (fix: App Settings toggle) apart from "user denied the prompt" (fix: browser site settings); both throw the same `NotAllowedError`. Current iOS honors the delegation (verified in iOS Safari); older iOS versions may block getUserMedia in cross-origin iframes, so a `<input type="file" accept="image/*" capture="environment">` fallback is still worth shipping.
